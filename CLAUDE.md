...
- **AI recipe suggestions**: `openAiSuggestModal()` calls a Supabase Edge Function (`supabase/functions/ai-suggest/index.ts`, a reference file like `schema.sql` — not auto-deployed) which proxies to the Anthropic Messages API using `ANTHROPIC_API_KEY` as a Supabase secret (the key must never live in client JS since this is a static site on GitHub Pages). The function validates the caller's Supabase JWT, then asks Claude (via forced tool-use, tool `return_suggestions`) to bucket suggestions into `noShopping`/`smallShopping`/`bigShopping` based on the current pantry, saved recipes, and dietary profiles sent in the request body. Each suggestion has `source: "existing"|"new"`; existing ones link to `openRecipeDetail()`, new ones can be persisted via `saveAiRecipe()` (inserts into `recipes` with `ai_generated: true`) and their `missingIngredients` can be pushed to the grocery list via `addMissingToGrocery()`. Suggestion payloads are cached client-side in `aiSuggestionCache` (keyed by a random card id) so the save/add-to-grocery buttons can look them up without re-fetching.
  - Deployment (manual, like the DB schema): `supabase functions deploy ai-suggest` and `supabase secrets set ANTHROPIC_API_KEY=...` (or set both via the Supabase Dashboard).
